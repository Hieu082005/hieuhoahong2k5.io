...
const run = async function ({ api, event }) {
    ...
    var gio2 = moment.tz("Europe/London").format("HH:mm:ss || D/MM/YYYY");
    ...
    // Check if args is defined and has necessary elements
    const pn = args && args[0] && args[0][0] ? args[0][0] + config.name : '';

    // Nếu args không hợp lệ, xử lý tương ứng
    if (!pn) {
        console.error('Error: args is undefined or has unexpected structure.');
        return;
    }
